...
const horseBodyValidators = [
  ...
  body('sireId').isString().trim().isLength({ min: 1, max: 255 }).withMessage('Sire is required'),
  ...
  body('horseTypeId').isString().trim().isLength({ min: 1, max: 64 }).withMessage('Horse type is required'),
  body('jurisdictionIds').isArray({ min: 1 }).withMessage('At least one jurisdiction is required'),
  body('jurisdictionIds.*').isString().trim().isLength({ min: 1, max: 64 }).withMessage('Invalid jurisdiction'),
  body('trainerId').optional({ values: 'falsy' }).isString().trim().isLength({ max: 255 }),
...
const horseUpdateValidators = [
  ...
  body('sireId').optional().isString().trim().isLength({ min: 1, max: 255 }),
  ...
  body('horseTypeId').optional().isString().trim().isLength({ min: 1, max: 64 }),
  body('jurisdictionIds').optional().isArray({ min: 1 }),
  body('jurisdictionIds.*').optional().isString().trim().isLength({ min: 1, max: 64 }),
  body('trainerId').optional({ values: 'falsy' }).isString().trim().isLength({ max: 255 }),
  body('stableLocation').optional().isString().trim().isLength({ max: 255 }),
